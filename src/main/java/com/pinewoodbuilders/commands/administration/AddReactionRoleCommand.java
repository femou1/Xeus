...
import com.pinewoodbuilders.database.transformers.GuildTypeTransformer;
...
public class AddReactionRoleCommand extends Command {
    ...
    public AddReactionRoleCommand(Xeus avaire) {
        super(avaire, false);
    }
        ...
        context.getChannel().getHistory().retrievePast(2).queue(messages -> {
            ...
            try {
                avaire.getDatabase().newQueryBuilder(Constants.REACTION_ROLES_TABLE_NAME)
                    ...
                    .update(statement -> {
                        statement.set("roles", Xeus.gson.toJson(reactionTransformer.getRoles()));
                    });
