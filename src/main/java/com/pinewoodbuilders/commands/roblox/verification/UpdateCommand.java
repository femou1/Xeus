...
public class UpdateCommand extends Command {
    ...
    private final Bandwidth limit = Bandwidth.simple(120, Duration.ofSeconds(60));
    ...
    private boolean updateEveryone(CommandMessage context) {
        ...
        if (verificationRunning) {
            context.makeError("A guild is already running a mass verification, due to rate-limits. You might get rate-limited.").queue();
        }

        context.makeWarning("Running verification...\nDepending on the member total (:members) this might take a while. You will be mentioned once the time is over.")
        ...
        for (Member member : context.getGuild().getMembers()) {
            if (!bucket.tryConsume(1)) {
                context.makeInfo("Hit bucket ratelimit, pausing for 30 seconds").setFooter("This message deletes after 25 seconds.").queue(m -> m.delete().queueAfter(25, TimeUnit.SECONDS));
                try {
                    Thread.sleep(30000);
                ...
                if (avaire.getRobloxAPIManager().getKronosManager().getTrelloBans().containsKey(verificationEntity.getRobloxId())) {
                    ...
                    if (banLabels.size() > 0) {
                        if (context.getMessage().getContentDisplay().contains("--pbac-trelloban-message")) {
                            ...
                            if (!canAppeal && isPermenant) {
                                context.guild.modifyMemberRoles(member, context.guild.getRoleById(trellobanRoleId)).queue();
                                member.getUser().openPrivateChannel().flatMap(u -> u.sendMessage("Loading ban message...")).flatMap(m -> m.editMessage(member.getAsMention())
                                    .setEmbeds(MessageFactory.makeSuccess(m, "[You have been trello-banned forever within Pinewood, this ban is permenant, so you're not allowed to appeal it. We wish you a very good day sir, and goodbye.](https://www.youtube.com/watch?v=BXUhfoUJjuQ)").buildEmbed())).queue();
            ...
            Map <GuildRobloxRanksService.GroupRankBinding, Role> bindingRoleMap =
                guildRanks.getGroupRankBindings().stream()
                    .collect(Collectors.toMap(Function.identity(), groupRankBinding -> context.guild.getRoleById(groupRankBinding.getRole()))),
            ...
            bindingRoleMap.forEach((groupRankBinding, role) -> {
                ...
                for (String groupRank : gamepassBinds) {
                    ...
                    gamepassBinds.stream().filter(group -> group.split(":")[0].equals("GamePass") && group.split(":")[1].equals(rankId)).forEach(gamepass -> {
                        List <RobloxGamePassService.Datum> rgs = avaire.getRobloxAPIManager().getUserAPI().getUserGamePass(verificationEntity.getRobloxId(), Long.valueOf(rankId));
                        if (rgs != null) {
                            bindingRoleAddMap.put(groupRankBinding, role);
                        }
                    });
            ...
            context.getGuild().modifyMemberRoles(member, rolesToAdd, rolesToRemove)
                .queue(l -> count.getAndIncrement(), null);
            ...
            if (!verificationEntity.getRobloxUsername().equals(member.getEffectiveName())) {
                if (PermissionUtil.canInteract(context.guild.getSelfMember(), member)) {
                    ...
                    ignoredMembers.put(member, "I do not have the permission to modify their nickname, or their highest rank is above mine.");
        ...
        context.getChannel().sendMessage(context.getMember().getAsMention()).setEmbeds(context.makeSuccess("All members have been updated").buildEmbed()).queue();
        ...
        builder.setText("All members that failed to update:")
    ...
    private boolean updateAllGuilds(CommandMessage context) {
        ...
        if (verificationRunning) {
            context.makeError("Verification is already being done in another guild, please wait until the verification has ended before starting another mass-verification.").queue();
            return false;
        }
        ...
        for (String guildId : Constants.guilds) {
            ...
            if (g != null) {
                ...
                for (Member member : g.getMembers()) {
                    ...
                    if (member.getRoles().stream().anyMatch(r -> r.getName().equalsIgnoreCase("Xeus Bypass") || r.getName().equalsIgnoreCase("RoVer Bypass"))) {
                        ignoredMembers.put(member, " has the Xeus/RoVer bypass role in `" + g.getName() + "`, this user cannot be verified/updated.");
                        continue;
                    }
                    ...
                    VerificationTransformer verificationTransformer = VerificationController.fetchVerificationFromGuild(avaire, g);
                    if (verificationTransformer == null) {
                        break;
                    }
                    ...
                    Map <GuildRobloxRanksService.GroupRankBinding, Role> bindingRoleMap =
                        guildRanks.getGroupRankBindings().stream()
                            .collect(Collectors.toMap(Function.identity(), groupRankBinding -> g.getRoleById(groupRankBinding.getRole()))),
                        bindingRoleAddMap = new HashMap <>();
                    ...
                    if (!verificationEntity.getRobloxUsername().equals(member.getEffectiveName())) {
                        if (PermissionUtil.canInteract(g.getSelfMember(), member)) {
                            g.modifyNickname(member, verificationTransformer.getNicknameFormat().replace("%USERNAME%", verificationEntity.getRobloxUsername())).queue();
                            //stringBuilder.append("\n\nNickname has been set to `").append(verificationEntity.getRobloxUsername()).append("`");
                        } else {
                            ignoredMembers.put(member, "I do not have the permission to modify their nickname, or their highest rank is above mine.");
                            //stringBuilder.append("\n\nChanging nickname failed :(");
                    ...
                    if (guildCount.get() % 100 == 0) {
                        context.makeInfo("**:guild**: `:currentGuildCount/:guildCount`\n**Global**: `:currentCount/:globalCount`")
                            .set("guild", g.getName())
                            .set("currentGuildCount", guildCount.get())
                            .set("currentCount", count.get())
                            .set("guildCount", g.getMembers().size()).set("globalCount", avaire.getShardManager().getUsers().size()).queue();
                    }
                }
                List <String> failedMembers = new ArrayList <>();
                ignoredMembers.forEach((m, r) -> {
