...
public class VerificationManager {
  ...
  public boolean verify(CommandMessage context, Member member, boolean useCache) {
    ...
    context.makeInfo("<a:loading:742658561414266890> Checking verification database <a:loading:742658561414266890>")
        .queue(originalMessage -> {
          if (member == null) {
            errorMessage(context, "Member entity doesn't exist. Verification cancelled on you", originalMessage);
            return;
          }
          ...
          if (context.getGuildSettingsTransformer().getPbVerificationTrelloban()) {
            ...
            Guild g = context.getGlobalSettingsTransformer() != null
                && context.getGlobalSettingsTransformer().getAppealsDiscordId() != 0
                    ? avaire.getShardManager()
                        .getGuildById(context.getGlobalSettingsTransformer().getAppealsDiscordId())
                    : null;
            ...
            if (trellobans != null) {
              if (avaire.getRobloxAPIManager().getKronosManager().getTrelloBans()
                ...
                if (banLabels.size() > 0) {
                  ...
                  if (!context.getGuild().getId().equals(g.getId())) {
                      ...
                      member.getUser().openPrivateChannel().flatMap(u -> u.sendMessage("Please open this message..."))
                          .flatMap(m -> m.editMessage(member.getAsMention())
                              .setEmbeds(MessageFactory.makeSuccess(m, "You have been trello-banned forever within `"
                                  + context.getGlobalSettingsTransformer().getGroupName()
                                  + "` however you are still allowed to appeal within the "
                                  + (g != null ? g.getName() : "Appeals guild not set") + ".\n\n"
                      ...
                      member.getUser().openPrivateChannel().flatMap(u -> u.sendMessage("Please open this message..."))
                          .flatMap(m -> m.editMessage(member.getAsMention())
                              .setEmbeds(MessageFactory.makeSuccess(m, "You have been trello-banned within`"
                                  + context.getGlobalSettingsTransformer().getGroupName()
                                  + "` however you are still allowed to appeal within the "
                                  + (g != null ? g.getName() : "Appeals guld not set.\n\n")
                                  + "Your trello-ban has the following labels, I'd suggest sharing these with your appeals handler:"
                                  + banLabels.stream().map(c -> "\n - " + c.getName()).collect(Collectors.joining()))
                                  .buildEmbed()))
                          .queue();
                    ...
                    if (!canAppeal && isPermenant) {
                      member.getUser().openPrivateChannel().flatMap(u -> u.sendMessage("Loading ban message..."))
                          .flatMap(m -> m.editMessage(member.getAsMention())
                              .setEmbeds(MessageFactory.makeSuccess(m, "[You have been trello-banned forever within "
                                  + context.getGlobalSettingsTransformer().getGroupName()
                                  + ", this ban is permenant, so you're not allowed to appeal it. We wish you a very good day sir, and goodbye.](https://www.youtube.com/watch?v=BXUhfoUJjuQ)")
                                  .buildEmbed()))
                          .queue();
                      avaire.getShardManager().getTextChannelById("778853992704507945").sendMessage("Loading...")
                          .flatMap(message -> message.editMessage("Shuted the fuck up.")
                    ...
                    if (mgmLogs != 0) {
                      ...
                      if (tc != null) {
                        tc.sendMessageEmbeds(context
                            .makeInfo("[``:global-unbanned-id`` has tried to verify in " + guild.getName()
                            ...
                            .set("reason",
                                banLabels.stream().map(c -> "\n - " + c.getName()).collect(Collectors.joining()))
                            .set("user", "XEUS AUTO BAN").set("link", context.getMessage().getJumpUrl()).buildEmbed())
                            .queue();
                      }
                    }
                    return;
                    ...
                    if (canAppeal) {
                      context.guild.modifyMemberRoles(context.member, context.guild.getRoleById(canAppealRoleId))
                          .queue();
                      member.getUser().openPrivateChannel().flatMap(u -> u.sendMessage("Please open this message..."))
                          .flatMap(m -> m.editMessage(member.getAsMention())
                              .setEmbeds(MessageFactory.makeSuccess(m, "You have been trello-banned within "
                                  + context.getGlobalSettingsTransformer().getGroupName()
                                  + ", [however you are still allowed to appeal within the " + g.getName() + "]().\n\n"
                    ...
                    if (!canAppeal && isPermenant) {
                      context.guild.modifyMemberRoles(context.member, context.guild.getRoleById(trellobanRoleId))
                          .queue();
                      member.getUser().openPrivateChannel().flatMap(u -> u.sendMessage("Loading ban message..."))
          ...
          try {
            Collection accounts = avaire.getDatabase().newQueryBuilder(Constants.ANTI_UNBAN_TABLE_NAME)
                ...
                .andWhere("main_group_id", context.getGuildSettingsTransformer().getMainGroupId()).get();
            if (accounts.size() > 0) {
                  ...
                  if (row.getBoolean("official_sub_group", false)) {
                    guild
                        .ban(member.getId(), time,
                            "Banned by: " + context.member.getEffectiveName() + "\n" + "For: " + reason
                        ...
                        .reason(
                            "Global Ban, executed by " + context.member.getEffectiveName() + "." + "For: \n" + reason)
                        .queue();
              ...
              if (avaire.getShardManager().getUserById(member.getId()) != null) {
                ...
                avaire.getShardManager().getUserById(member.getId()).openPrivateChannel().queue(p -> {
                  p.sendMessageEmbeds(context.makeInfo(
                      ...
                      .set("groupLink",
                          "https://roblox.com/groups/" + context.getGlobalSettingsTransformer().getMainGroupId())
                      .buildEmbed()).queue();
                });
              }
              ...
              if (context.getGuildSettingsTransformer() != null) {
                ...
                if (transformer.getUserAlertsChannelId() != 0
                    && context.getGuild().getTextChannelById(transformer.getUserAlertsChannelId()) != null) {
                  MessageFactory
                      .makeEmbeddedMessage(context.getGuild().getTextChannelById(transformer.getUserAlertsChannelId()),
                          new Color(255, 0, 0))
          ...
          if (robloxRanks == null || robloxRanks.size() == 0) {
            errorMessage(context,
                verificationEntity.getRobloxUsername() + " does not have any ranks or groups on his name.",
                originalMessage);
            return;
          }
          ...
          java.util.Collection<Role> rolesToAdd = bindingRoleAddMap.values().stream()
              .filter(role -> RoleUtil.canBotInteractWithRole(context.getMessage(), role)).collect(Collectors.toList()),
              rolesToRemove = bindingRoleMap.values().stream().filter(role -> !bindingRoleAddMap.containsValue(role)
                  && RoleUtil.canBotInteractWithRole(context.getMessage(), role)).collect(Collectors.toList());
          ...
          originalMessage.editMessageEmbeds(context.makeSuccess(stringBuilder.toString())
              .setThumbnail(getImageFromVerificationEntity(verificationEntity)).buildEmbed()).queue();
        });
  ...
  private boolean errorMessage(CommandMessage em, String s, Message mess) {
    mess.editMessageEmbeds(
        em.makeError(s).setTitle("Error during verification!").requestedBy(em).setTimestamp(Instant.now()).buildEmbed())
        .queue();
