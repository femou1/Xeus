...
public class CreateGlobalWatchTableMigration implements Migration {

    @Override
    public String created_at() {
        return "Sat, Jan 15, 2022 12:51 PM";
    }

    @Override
    public boolean up(Schema schema) throws SQLException {
        return schema.createIfNotExists(Constants.GLOBAL_WATCH_TABLE_NAME, table -> {
            table.Long("guild_id").unsigned();
            table.Long("modlog_id").unsigned();
            table.DateTime("expires_in").nullable();
            table.Long("main_group_id").unsigned();
            table.Timestamps();
        });
    }
    ...
    public boolean down(Schema schema) throws SQLException {
        return schema.dropIfExists(Constants.GLOBAL_MUTE_TABLE_NAME);
    }
}
