...
public class AddPardonToMGMLogTableMigration implements Migration {

    @Override
    public String created_at() {
        return "Tue, Nov 25, 2021 13:00 PM";
    }

    @Override
    public boolean up(Schema schema) throws SQLException {
        if (schema.hasColumn(Constants.MGM_LOG_TABLE_NAME, "pardon")) {
            return true;
        }

        schema.getDbm().queryUpdate(String.format(
            "ALTER TABLE `%s` ADD `pardon` TINYINT(1) NOT NULL DEFAULT '0';",
            Constants.MGM_LOG_TABLE_NAME
        ));
